...
def main():
    ...
    #     results_path = Path(args.model_path).parent
    ...
    if not args.test:
        ...
        print("Test accuracy:", sklearn.metrics.accuracy_score(data.obs["cell_labels"], prediction))
        print("Test f1 score:", sklearn.metrics.f1_score(data.obs["cell_labels"], prediction, average="macro"))
        
        pred_df = pd.DataFrame(probs, columns=data.obs["cell_labels"].cat.categories)
        pred_df['predicted_labels'] = prediction
        pred_df["true_labels"] = data.obs["cell_labels"].values
        pred_df.to_csv(results_path / "test_predictions.tsv", index=False, sep="\t")
